...
func (c *AdminRedFlagController) Create(ctx *gin.Context) {
	...
	if err := ctx.ShouldBindJSON(&in); err != nil { ctx.JSON(http.StatusBadRequest, gin.H{"error": "invalid body"}); return }
...
func (c *AdminRedFlagController) Delete(ctx *gin.Context) {
	id := ctx.Param("id")
	actor := ctx.GetString("userID")
	if err := c.uc.Delete(ctx.Request.Context(), id, actor); err != nil { HandleHTTPError(ctx, err); return }
